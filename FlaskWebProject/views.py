...
def authorized():
    ...
    if "error" in request.args:  # Authentication/Authorization failure
        app.logger.warning('Invalid login attempt via Microsoft sign-in: %s', request.args.get('error'))
    ...
    if request.args.get('code'):
        ...
        if "error" in result:
            app.logger.warning('Invalid login attempt via Microsoft sign-in: %s', result.get('error'))
            return render_template("auth_error.html", result=result)
        session["user"] = result.get("id_token_claims")
        # Note: In a real app, we'd use the 'name' property from session["user"] below
        # Here, we'll use the admin username for anyone who is authenticated by MS
        user = User.query.filter_by(username="admin").first()
        app.logger.info('%s logged in successfully via Microsoft sign-in', user.username)
        login_user(user)
        _save_cache(cache)
    return redirect(url_for('home'))
